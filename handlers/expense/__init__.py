...
"Handler: fetch command to add expense and send expense categories."
@router.message(Text(lines.keyboards_lines['menu_keyboard']['expense']))
async def operation_type_handler(message: Message, state: FSMContext, bot=config.bot) -> None:
    ...
    storage.create_operation_in_storage(user_id=u.fetch_user_id(message), operation_type='expense', currency=database.fetch_user_currency(u.fetch_user_id(message)))

    expense_categories: dict = database.load_categories(user_id=u.fetch_user_id(message), operation_type='expense')

    await message.answer(text=lines.new_operation_lines['text_choose_income_category'],
                         reply_markup=create_categories_keyboard(categories=expense_categories))

    await state.set_state(AddExpense.set_expense_category)


"Handler: fetch expense category from callback and add it in bot storage."
...
async def expense_category_handler(callback: CallbackQuery, state: FSMContext, bot=config.bot) -> None:
    ...
    expense_categories: dict = database.load_categories(user_id=u.fetch_user_id(callback), operation_type='expense')
    ...
    storage.update_storage_data(user_id, key='emoji', value=emoji.emojize(expense_categories[operation_category]['emoji']))
