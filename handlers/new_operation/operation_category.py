...
async def operation_category_handler(callback_query: CallbackQuery, state: FSMContext, bot=bot):
    # Remove callback delay.
    await handlers.remove_callback_delay(callback_query=callback_query)

    # Fetch category from callback.
    category: str = callback_query.data.split('_')[1]
    ...
    await bot.edit_message_text(text=new_operation['operation_category_chosen'](category=category),
    ...
    await bot.send_message(chat_id=callback_query.message.chat.id,
                           text=new_operation['choose_operation_value'])
