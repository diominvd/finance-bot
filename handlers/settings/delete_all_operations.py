...
async def delete_all_operations_handler(message: Message, state: FSMContext):
    ...
    operations_list: list = database.select_operations(user_id=handlers.fetch_user_id(obj=message))
    ...
    if len(operations_list) == 0:
        ...
        database.delete_all_operations(user_id=handlers.fetch_user_id(obj=message))
